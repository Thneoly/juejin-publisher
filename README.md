...
| 发布成功但前台 404、专栏显示空 | `status:0`=审核中，属正常；1/2 才上线；`audit_status:-1`=驳回 |
| 审核被按「推广类」驳回 | 正文含招聘平台 URL/域名字符串+薪资数字；**标题含「高薪/招聘」组合同样触发**——标题用金句钩子，删旧文重发（实测有效） |
| 专栏挂不上 | 发布体的 `column_ids` 字段；专栏创建无 API，走创作者中心手动建一次 |
